...
function RecruiterHeaderContent() {
  return (
    <Suspense
      fallback={
        <div className="flex h-16 items-center justify-between px-6 bg-gray-900 text-white">
          <div className="flex items-center gap-6">
            <Link
              href="/recruiter/event-selection"
    ...
    >
      <RecruiterHeaderInner />
    </Suspense>
  );
}

async function RecruiterHeaderInner() {
  // 認証チェックはレイアウトで実施済み
  ...
  return (
    <div className="flex h-16 items-center justify-between px-6 bg-gray-900 text-white">
      <div className="flex items-center gap-6">
        <Link href="/recruiter/event-selection" className="flex items-center gap-2 font-semibold">
          <span>イベント運営システム</span>
        </Link>
      </div>
      <div className="flex items-center gap-4">
